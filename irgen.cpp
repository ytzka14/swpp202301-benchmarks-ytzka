...
static cl::opt<string> optInput(cl::Positional,
                                cl::desc("<input bitcode file>"), cl::Required,
                                cl::value_desc("filename"),
                                cl::cat(optCategory));

static cl::opt<string> optOutput(cl::Positional, cl::desc("output ll file"),
...
class RemoveUnsupportedOps : public llvm::PassInfoMixin<RemoveUnsupportedOps> {
  Constant *getZero(Type *T) {
    return T->isPointerTy() ? ConstantPointerNull::get(dyn_cast<PointerType>(T))
                            : ConstantInt::get(T, 0);
  }
    ...
    for (auto &BB : F) {
      for (auto &I : BB) {
        ...
        if (auto *Intr = dyn_cast<IntrinsicInst>(II)) {
          ...
          auto *RE = F.getReturnType()->isVoidTy()
                         ? ReturnInst::Create(Cxt)
      ...
      if (auto *F = dyn_cast<Function>(&G)) {
        ...
        if (GV->hasInitializer()) {
          if (GV->getValueType()->isIntegerTy()) {
            ...
            errs()
                << "WARNING: Global variables should not have initializers!\n";
            errs() << "\t" << *GV << "\n";
          }
        }
      }
  ...
  PreservedAnalyses run(Module &M, ModuleAnalysisManager &MAM) {
    for (auto &F : M) {
      if (F.isDeclaration()) {
        ...
        // F.addAttributes(AttributeList::ReturnIndex, Attribute::NoAlias);
        continue;
      }
